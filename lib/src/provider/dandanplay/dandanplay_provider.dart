...
final class DandanplayAnimeId {
  const DandanplayAnimeId(this.value) : assert(value != '', 'Dandanplay anime id must not be empty.');

  final String value;
}

final class DandanplayEpisodeId {
  const DandanplayEpisodeId(this.value) : assert(value != '', 'Dandanplay episode id must not be empty.');
...
abstract interface class DandanplayProvider implements GatewayBoundProvider {
  ...
  Future<AcgProviderResult<List<DandanplayMatchCandidate>>> matchLocalMedia(String filename);

  Future<AcgProviderResult<List<DandanplayMatchCandidate>>> search(String query);
}
