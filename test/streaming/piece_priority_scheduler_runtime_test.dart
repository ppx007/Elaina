...
void main() {
  group('Task 1.1 - plan generation from persisted state', () {
    test(
        'full plan from metadata, files, streams, buffers, windows, seeks, profiles',
        () async {
      ...
      expect(
          o.plan!.rules.map((r) => r.pieceIndex.value), containsAll([1, 2, 4]));
      expect(
          o.plan!.rules
              .where((r) => r.reason == PiecePriorityRuleReason.seekTarget)
              .single
              .priority,
      ...
      expect(
          (await h.ss.activeProfile(taskId: 'task-1', streamId: 'task-1::1'))
              ?.profileId,
          'balanced');
    ...
    test('playback only', () async {
      ...
      expect(
          o.plan!.rules
              .where((r) => r.reason == PiecePriorityRuleReason.playbackWindow)
              .isNotEmpty,
          isTrue);
      await h.bus.close();
    });
    ...
    test('alt profile', () async {
      ...
      final o = await h.scheduler.plan(PiecePriorityPlanRequest(
        taskId: const BtTaskId('task-1'),
        streamId: const VirtualMediaStreamId('task-1::1'),
        profile: const PiecePriorityStrategyProfile(
            id: 'aggressive',
            displayName: 'Aggressive',
            firstPiecePriority: DownloadPriority.critical,
            tailPiecePriority: DownloadPriority.critical,
            playbackWindowPriority: DownloadPriority.critical,
            seekTargetPriority: DownloadPriority.critical,
            staleWindowPriority: DownloadPriority.normal,
            lookaheadBytes: 4096,
            seekLookaheadBytes: 2048,
            edgePieceCount: 2),
      ));
      expect(o.isSuccess, isTrue);
      expect(o.plan!.profileId, 'aggressive');
      expect(
          o.plan!.rules.every((r) => r.priority == DownloadPriority.critical),
          isTrue);
      await h.bus.close();
    });

    test('derive file-piece map', () {
      final m = deriveFilePieceMap(
  ...
  group('Task 1.2 - typed planning failures', () {
    test('metadataUnavailable', () async {
      ...
      final s = DeterministicPiecePriorityScheduler(
          btTaskStore: bt,
          streamStore: ss,
          schedulerStore: DeterministicPiecePrioritySchedulerStore());
      final o = await s.plan(PiecePriorityPlanRequest(
          taskId: const BtTaskId('task-1'),
          streamId: const VirtualMediaStreamId('task-1::1'),
          profile: _prof()));
    ...
    test('fileMapUnavailable when missing', () async {
      ...
      await bt.storeMetadata(const StoredBtTaskMetadataRecord(
          taskId: 'task-1',
          infoHash: 'abc',
          name: 'Pack',
          totalSizeBytes: 6144,
    ...
    test('fileMapUnavailable when skipped', () async {
      ...
      await bt.storeMetadata(const StoredBtTaskMetadataRecord(
          ...
          totalSizeBytes: 6144,
          pieceLengthBytes: 1024));
      await bt.storeFiles(taskId: 'task-1', files: const [
    ...
    test('streamUnavailable when taskId mismatch', () async {
      ...
      await bt.storeMetadata(const StoredBtTaskMetadataRecord(
          ...
          totalSizeBytes: 6144,
          pieceLengthBytes: 1024));
      await ss.storeStream(StoredVirtualMediaStreamRecord(
          id: 'task-1::1',
          taskId: 'task-999',
          fileIndex: 1,
          lengthBytes: 4096,
    ...
    test('streamFailed via runtime plan', () async {
      ...
      await ss.storeStream(StoredVirtualMediaStreamRecord(
          ...
          createdAt: _dt(),
          updatedAt: _dt()));
      final rt = PiecePrioritySchedulerRuntime(
          btTaskStore: bt,
          streamStore: ss,
          schedulerStore: DeterministicPiecePrioritySchedulerStore(),
          profiles: [_prof()],
          clock: () => _dt());
      final o = await rt.plan(PiecePriorityPlanRequest(
          taskId: const BtTaskId('task-1'),
          streamId: const VirtualMediaStreamId('task-1::1'),
          profile: _prof()));
      expect(o.isSuccess, isFalse);
    ...
    test('streamFailed via runtime planWithProfileId', () async {
      ...
      final rt = PiecePrioritySchedulerRuntime(
          ...
          profiles: [_prof()],
          clock: () => _dt());
      final o = await rt.planWithProfileId(
          taskId: const BtTaskId('task-1'),
          streamId: const VirtualMediaStreamId('task-1::1'),
          profileId: 'balanced');
      expect(o.isSuccess, isFalse);
    ...
    test('wrong stream playback window', () async {
      ...
      final o = await h.scheduler.plan(PiecePriorityPlanRequest(
          ...
          profile: _prof(),
          playbackWindow: const PlaybackWindow(
              streamId: VirtualMediaStreamId('task-1::999'),
              currentByteOffset: 0,
              lookaheadBytes: 1024)));
      expect(o.isSuccess, isFalse);
      expect(o.failure!.kind, PiecePriorityPlanFailureKind.streamUnavailable);
    ...
    test('wrong seek stream', () async {
      ...
      final o = await h.scheduler.plan(PiecePriorityPlanRequest(
          ...
          profile: _prof(),
          seekTarget: const SeekTarget(
              streamId: VirtualMediaStreamId('task-1::888'),
              targetByteOffset: 1024)));
      expect(o.isSuccess, isFalse);
      expect(o.failure!.kind, PiecePriorityPlanFailureKind.streamUnavailable);
      await h.bus.close();
    });
    ...
    test('publishes rejection on failure', () async {
      ...
      await s.plan(PiecePriorityPlanRequest(
          taskId: const BtTaskId('task-1'),
          streamId: const VirtualMediaStreamId('task-1::1'),
          profile: _prof()));
      final d = await f;
    ...
    test('disposed runtime rejects selectProfile', () async {
      ...
      final rt = PiecePrioritySchedulerRuntime(
          ...
          clock: () => _dt());
      await rt.dispose();
      final o = await rt.selectProfile(
          taskId: const BtTaskId('task-1'),
          streamId: const VirtualMediaStreamId('task-1::1'),
          profileId: 'balanced');
      expect(o.isSuccess, isFalse);
    ...
    test('disposed runtime rejects applyPlan', () async {
      ...
      final rt = PiecePrioritySchedulerRuntime(
          ...
          clock: () => _dt());
      await rt.dispose();
      final o = await rt.applyPlan(
          planId: const PiecePriorityPlanId('any-plan'),
  ...
  group('Task 1.3 - adapter-neutral application outcomes', () {
    test('accepted', () async {
      final h = await _harness();
      final pl = await h.scheduler.plan(PiecePriorityPlanRequest(
          taskId: const BtTaskId('task-1'),
          streamId: const VirtualMediaStreamId('task-1::1'),
          profile: _prof(),
          playbackWindow: const PlaybackWindow(
              ...
              currentByteOffset: 0,
              lookaheadBytes: 1024)));
      final rec = DeterministicPiecePriorityPlanApplicationRecorder(
          schedulerStore: h.ss,
          cacheInvalidationBus: h.bus,
          clock: () => _dt());
      final f = h.bus.events.take(1).toList();
      final a = await rec.applyAndRecord(
          planId: pl.plan!.id, applier: const _AcceptingApplier());
      final d = await f;
      expect(a.isSuccess, isTrue);
      expect((await h.ss.latestApplicationEvent(pl.plan!.id.value))?.outcome,
    ...
    test('rejected', () async {
      ...
      final rec = DeterministicPiecePriorityPlanApplicationRecorder(
          ...
          clock: () => _dt());
      final f = h.bus.events.take(1).toList();
      final r = await rec.applyAndRecord(
          planId: pl.plan!.id, applier: const _RejectingApplier());
      final d = await f;
      expect(r.isSuccess, isFalse);
      expect(
          r.failure!.kind, PiecePriorityApplicationFailureKind.adapterRejected);
      expect((await h.ss.latestApplicationEvent(pl.plan!.id.value))?.outcome,
    ...
    test('unavailable applier', () async {
      ...
      expect(u.failure!.kind,
          PiecePriorityApplicationFailureKind.applierUnavailable);
      expect((await h.ss.latestApplicationEvent(pl.plan!.id.value))?.outcome,
          StoredPiecePriorityApplicationOutcomeKind.unavailable);
      await h.bus.close();
    });
    ...
    test('planNotFound', () async {
      ...
      final rec = DeterministicPiecePriorityPlanApplicationRecorder(
          schedulerStore: s, clock: () => _dt());
      final o = await rec.applyAndRecord(
          planId: const PiecePriorityPlanId('nonexistent'),
          applier: const _AcceptingApplier());
      expect(o.isSuccess, isFalse);
    ...
    test('replayable after restart', () async {
      ...
      final pl = await h.scheduler.plan(PiecePriorityPlanRequest(
          ...
          playbackWindow: const PlaybackWindow(
              ...
              currentByteOffset: 0,
              lookaheadBytes: 1024)));
      await DeterministicPiecePriorityPlanApplicationRecorder(
              schedulerStore: h.ss, clock: () => _dt())
          .applyAndRecord(
              planId: pl.plan!.id, applier: const _AcceptingApplier());
      expect((await h.ss.latestApplicationEvent(pl.plan!.id.value))?.outcome,
          StoredPiecePriorityApplicationOutcomeKind.accepted);
      await DeterministicPiecePriorityPlanApplicationRecorder(
              schedulerStore: h.ss,
              clock: () => DateTime.utc(2026, 6, 5, 12, 1))
          .applyAndRecord(
              planId: pl.plan!.id, applier: const _RejectingApplier());
      expect((await h.ss.latestApplicationEvent(pl.plan!.id.value))?.outcome,
          StoredPiecePriorityApplicationOutcomeKind.rejected);
  ...
  group('Task 1.4 - immutable snapshots and restart projections', () {
    test('active profile reconstructable', () async {
      final h = await _harness();
      await h.scheduler.plan(PiecePriorityPlanRequest(
          taskId: const BtTaskId('task-1'),
          streamId: const VirtualMediaStreamId('task-1::1'),
          profile: _prof()));
      final a =
    ...
    test('ordered rules reconstructable', () async {
      ...
      for (var i = 1; i < r.length; i++) {
        expect(r[i].order, greaterThanOrEqualTo(r[i - 1].order));
      }
      await h.bus.close();
    });
    ...
    test('unavailable state reconstructable', () async {
      ...
      await DeterministicPiecePriorityPlanApplicationRecorder(
          schedulerStore: h.ss,
          clock: () => _dt()).applyAndRecord(planId: pl.plan!.id);
      final e = await h.ss.latestApplicationEvent(pl.plan!.id.value);
      expect(e, isNotNull);
    ...
    test('multi-plan projection', () async {
      ...
      final p1 = await h.scheduler.plan(PiecePriorityPlanRequest(
          taskId: const BtTaskId('task-1'),
          streamId: const VirtualMediaStreamId('task-1::1'),
          profile: _prof()));
      ...
      await bt2.storeMetadata(const StoredBtTaskMetadataRecord(
          taskId: 'task-1',
          infoHash: 'abc',
          name: 'Episode Pack',
          totalSizeBytes: 6144,
          pieceLengthBytes: 1024));
      await bt2.storeFiles(taskId: 'task-1', files: const [
        StoredBtTaskFileRecord(
            taskId: 'task-1',
            index: 1,
            path: 'Episode 2.mkv',
      ...
      final s2 = DeterministicPiecePriorityScheduler(
          btTaskStore: bt2,
          streamStore: ss2,
          schedulerStore: h.ss,
          clock: () => DateTime.utc(2026, 6, 5, 13));
      await s2.plan(PiecePriorityPlanRequest(
          taskId: const BtTaskId('task-1'),
          streamId: const VirtualMediaStreamId('task-1::1'),
          profile: const PiecePriorityStrategyProfile(
              id: 'seq',
              displayName: 'Seq',
              firstPiecePriority: DownloadPriority.high,
              tailPiecePriority: DownloadPriority.low,
              playbackWindowPriority: DownloadPriority.high,
              seekTargetPriority: DownloadPriority.normal,
              lookaheadBytes: 1024,
              edgePieceCount: 1)));
      final l = await h.ss
          .latestPlanForStream(taskId: 'task-1', streamId: 'task-1::1');
...
final class _RuntimeHarness {
  const _RuntimeHarness(
      {required this.scheduler, required this.ss, required this.bus});
...
Future<_RuntimeHarness> _harness({int? bufferedRangeEnd}) async {
  ...
  if (bufferedRangeEnd != null) {
    await ss.recordBufferedRange(StoredVirtualStreamBufferedRangeRecord(
        streamId: 'task-1::1',
        startByte: 0,
...
PiecePriorityStrategyProfile _prof() => const PiecePriorityStrategyProfile(
    id: 'balanced',
...
StoredBtTaskRecord _tr() => StoredBtTaskRecord(
    id: 'task-1',
...
StoredVirtualMediaStreamRecord _sr() => StoredVirtualMediaStreamRecord(
    id: 'task-1::1',
    taskId: 'task-1',
    fileIndex: 1,
    lengthBytes: 4096,
...
final class _AcceptingApplier implements PiecePriorityPlanApplier {
  ...
  Future<PiecePriorityApplicationOutcome> apply(PiecePriorityPlan plan) =>
      Future.value(const PiecePriorityApplicationOutcome.accepted());
}
...
final class _RejectingApplier implements PiecePriorityPlanApplier {
  ...
  @override
  Future<PiecePriorityApplicationOutcome> apply(PiecePriorityPlan plan) =>
      Future.value(const PiecePriorityApplicationOutcome.rejected(
