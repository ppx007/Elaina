...
$requiredFiles = @(
  ...
  'tools/av_sync_guard_runtime_check.dart'
)
...
# 2. Dart smoke check
# ---------------------------------------------------------------------------
& dart run (Join-Path $root 'tools/av_sync_guard_runtime_check.dart')
...
# ---------------------------------------------------------------------------
$checkerPath = Join-Path $root 'tools/av_sync_guard_runtime_check.dart'
...
foreach ($term in $testAndCheckerForbiddenTerms) {
  ...
  if ($checker -match [regex]::Escape($term)) {
    throw "Forbidden AV sync guard boundary term '$term' found in av_sync_guard_runtime_check.dart"
  }
}
