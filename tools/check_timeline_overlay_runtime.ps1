...
$requiredFiles = @(
  ...
  'tools/timeline_overlay_runtime_check.dart'
)
...
# 2. Dart smoke check
# ---------------------------------------------------------------------------
& dart run (Join-Path $root 'tools/timeline_overlay_runtime_check.dart')
...
# ---------------------------------------------------------------------------
$checkerPath = Join-Path $root 'tools/timeline_overlay_runtime_check.dart'
...
foreach ($term in $testAndCheckerForbiddenTerms) {
  ...
  if ($checker -match [regex]::Escape($term)) {
    throw "Forbidden timeline overlay boundary term '$term' found in timeline_overlay_runtime_check.dart"
  }
}
