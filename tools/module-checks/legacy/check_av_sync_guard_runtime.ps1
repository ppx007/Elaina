...
$requiredFiles = @(
  ...
  'lib/src/playback/av_sync_guard_runtime.dart',
  'test/playback/av_sync_guard_runtime_test.dart',
  'tools/runtime_checks/av_sync_guard_runtime_contract.dart'
)

foreach ($file in $requiredFiles) {
  $path = Join-Path $root $file
  if (-not (Test-Path -LiteralPath $path)) {
    throw "Missing required AV sync guard runtime file: $file"
  }
}
...
# 2. Dart smoke check
# ---------------------------------------------------------------------------
& dart run (Join-Path $root 'tools/runtime_checks/av_sync_guard_runtime_contract.dart')
if ($LASTEXITCODE -ne 0) {
  throw "AV sync guard runtime Dart checker failed with exit code $LASTEXITCODE"
}

# ---------------------------------------------------------------------------
# 3. Required-term checks in runtime surface
# ---------------------------------------------------------------------------
$runtimePath = Join-Path $root 'lib/src/playback/av_sync_guard_runtime.dart'
$runtime = Get-Content -LiteralPath $runtimePath -Raw

$requiredRuntimeTerms = @(
  'AVSyncGuardBootstrap',
  'AVSyncGuardRuntime',
  'AVSyncGuardRuntimeProjection',
  'AVSyncGuardRuntimeRestartProjection',
  'AVSyncGuardRuntimeFailure',
  'AVSyncGuardRuntimeFailureKind',
  'AVSyncGuardRuntimeActionResult',
  'ingestSample(',
  'requestDegradation(',
  'checkRecovery(',
  'snapshot(',
...
foreach ($term in $requiredRuntimeTerms) {
  if ($runtime -notmatch [regex]::Escape($term)) {
    throw "AV sync guard runtime missing required term: $term"
  }
}
...
# ---------------------------------------------------------------------------
$checkerPath = Join-Path $root 'tools/runtime_checks/av_sync_guard_runtime_contract.dart'
...
foreach ($term in @(
  ...
  'AVSyncGuardBootstrap',
  'AVSyncGuardRuntimeFailureKind',
  'AVSyncGuardRuntime.unavailable',
  'dispose()',
  'DateTime.utc(2026, 6, 15, 12)'
)) {
  if ($checker -notmatch [regex]::Escape($term)) {
    throw "AV sync guard runtime checker missing required term: $term"
  }
}
...
foreach ($term in $runtimeForbiddenTerms) {
  if ($runtime -match [regex]::Escape($term)) {
    throw "Forbidden AV sync guard runtime boundary term '$term' found in av_sync_guard_runtime.dart"
  }
}

$runtimeTestPath = Join-Path $root 'test/playback/av_sync_guard_runtime_test.dart'
...
foreach ($term in $testAndCheckerForbiddenTerms) {
  if ($runtimeTest -match [regex]::Escape($term)) {
    throw "Forbidden AV sync guard boundary term '$term' found in av_sync_guard_runtime_test.dart"
  }
  if ($checker -match [regex]::Escape($term)) {
    throw "Forbidden AV sync guard boundary term '$term' found in av_sync_guard_runtime_contract.dart"
  }
}

# ---------------------------------------------------------------------------
# 7. Scope guard: runtime imports stay on Step 23 contracts
# ---------------------------------------------------------------------------
...
$forbiddenImports = @(
  ...
  'shader',
  'native_renderer',
  'video_enhancement_pipeline_runtime',
  'diagnostics_center',
  'rss_auto_download',
  'online_rule_runtime',
  'network_policy',
...
foreach ($import in $forbiddenImports) {
  if ($runtimeImports -match [regex]::Escape($import)) {
    throw "AV sync guard runtime must not import boundary surface: $import"
  }
}

Write-Output 'AV sync guard runtime checks passed.'
