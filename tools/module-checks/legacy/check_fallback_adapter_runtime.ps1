...
$requiredFiles = @(
  ...
  'tools/runtime_checks/fallback_adapter_runtime_contract.dart'
)
...
# 2. Dart smoke check
# ---------------------------------------------------------------------------
& dart run (Join-Path $root 'tools/runtime_checks/fallback_adapter_runtime_contract.dart')
...
# ---------------------------------------------------------------------------
$checkerPath = Join-Path $root 'tools/runtime_checks/fallback_adapter_runtime_contract.dart'
...
foreach ($term in $testAndCheckerForbiddenTerms) {
  ...
  if ($checker -match [regex]::Escape($term)) {
    throw "Forbidden fallback adapter boundary term '$term' found in fallback_adapter_runtime_contract.dart"
  }
}
