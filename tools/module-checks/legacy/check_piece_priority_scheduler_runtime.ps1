...
$requiredFiles = @(
  ...
  'tools/runtime_checks/piece_priority_scheduler_runtime_contract.dart'
)
...
# 2. Dart smoke check
# ---------------------------------------------------------------------------
& dart (Join-Path $root 'tools/runtime_checks/piece_priority_scheduler_runtime_contract.dart')
...
# ---------------------------------------------------------------------------
$checkerPath = Join-Path $root 'tools/runtime_checks/piece_priority_scheduler_runtime_contract.dart'
$checker = Get-Content -LiteralPath $checkerPath -Raw

foreach ($term in $forbiddenTestTerms) {
  if ($checker -match [regex]::Escape($term)) {
    throw "Forbidden Step 21+ dependency '$term' found in piece_priority_scheduler_runtime_contract.dart"
  }
}
