...
$requiredFiles = @(
  ...
  'tools/runtime_checks/video_enhancement_pipeline_runtime_contract.dart'
)
...
# 2. Dart smoke check
# ---------------------------------------------------------------------------
& dart run (Join-Path $root 'tools/runtime_checks/video_enhancement_pipeline_runtime_contract.dart')
...
# ---------------------------------------------------------------------------
$checkerPath = Join-Path $root 'tools/runtime_checks/video_enhancement_pipeline_runtime_contract.dart'
...
foreach ($term in $testAndCheckerForbiddenTerms) {
  ...
  if ($checker -match [regex]::Escape($term)) {
    throw "Forbidden video enhancement boundary term '$term' found in video_enhancement_pipeline_runtime_contract.dart"
  }
}
