...
Future<void> main() async {
  ...
  final OnlineRuleSourceRuntimeActionResult<OnlineRuleSourceRuntimeProjection> snapshot = await runtime.snapshot('source-1');
  _expect(snapshot.isSuccess, 'Initial snapshot must succeed.');
  final OnlineRuleSourceRuntimeRestartProjection restart = snapshot.value!.restart;
  _expect(restart.manifestValidationState == StoredOnlineRuleValidationState.valid,
      'Restart projection must show valid state.');

  final validateResult = await runtime.validate('source-1', OnlineRuleManifest(
  ...
  _expect(validateResult.isSuccess, 'Validate must succeed for valid manifest.');

  final evalResult = await runtime.evaluate('source-1', OnlineRuleEvaluationRequest(
    ...
    document: 'title="Check Title" detailUri="https://source.example.test/detail"',
  ));
  ...
  _expect(disableResult.value?.validationState == StoredOnlineRuleValidationState.disabled,
  ...
  _expect(reenableResult.value?.validationState == StoredOnlineRuleValidationState.valid,
      'Reenable must restore to valid.');

  final unsupportedRuntime = OnlineRuleSourceRuntime.unavailable(reason: 'No platform');
  _expectFailure((await unsupportedRuntime.snapshot('source-1')).isSuccess,
      'Unavailable snapshot must fail.');
  _expect((await unsupportedRuntime.snapshot('source-1')).failure?.kind == OnlineRuleSourceRuntimeFailureKind.unavailable,
      'Unavailable snapshot must report unavailable failure.');
  _expectFailure((await unsupportedRuntime.validate('source-1', OnlineRuleManifest(
  ...
  ))).isSuccess, 'Unavailable validate must fail.');
  _expectFailure((await unsupportedRuntime.evaluate('source-1', OnlineRuleEvaluationRequest(
    manifest: OnlineRuleManifest(
      sourceId: const OnlineRuleSourceId('s'),
      displayName: 'X',
      version: const OnlineRuleManifestVersion('1'),
    ...
    document: 'x',
  ))).isSuccess, 'Unavailable evaluate must fail.');
  ...
  await bus.close();
  print('Online rule source runtime check passed.');
}
